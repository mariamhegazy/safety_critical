...
def _prepare_sampling_schedule(args, device):
    """
    Wan checkpoints require the UniPC schedule distributed with the model.
    Reuse it to extract both the sigma trajectory and the exact timestep tensor.
    """
    ...
    if grpo_sample:
        ...
        for i in progress_bar:  # Add progress bar
            ...
            if isinstance(timestep_value, torch.Tensor):
                timestep_value = int(timestep_value.item())
            else:
...
def sample_reference_model(
    ...
    sigma_schedule, timestep_schedule, init_noise_sigma = _prepare_sampling_schedule(
        args, device
    )
    assert_eq(
    ...
    video_filename_prefix = getattr(args, "video_filename_prefix", "wan_2_1")
    ...
    if args.init_same_noise:
        input_latents = _sample_latents((1, IN_CHANNELS, latent_t, latent_h, latent_w))
    ...
    for index, batch_idx in enumerate(batch_indices):
        ...
        if video_frames.ndim == 5:
            # (batch, frames, channels, height, width) -> remove batch dim
        ...
        if args.use_hpsv2:
            with torch.no_grad():
                if video_path is not None:
                    pil_image = video_first_frame_to_pil(str(video_path))
                else:
                    first_frame = video_frames[0]
                    frame_uint8 = (np.clip(first_frame, 0.0, 1.0) * 255).astype(
                        np.uint8
                    )
                    pil_image = Image.fromarray(frame_uint8)
                image = (
                    preprocess_val(pil_image)
                    .unsqueeze(0)
    ...
    return all_rewards, all_latents, all_log_probs, sigma_schedule, timestep_schedule
...
def train_one_step(
    ...
    reward, all_latents, all_log_probs, sigma_schedule, timestep_schedule = (
    ...
    timesteps = (
        timestep_schedule.to(all_latents.device, dtype=torch.long)
        .unsqueeze(0)
        .repeat(batch_size, 1)
    )
...
if __name__ == "__main__":
    ...
    video_group.add_argument(
        ...
        dest="save_videos",
        action="store_true",
        help="Save decoded samples as mp4 files.",
    )
    video_group.add_argument(
        "--no_save_videos",
        dest="save_videos",
        action="store_false",
        help="Disable saving decoded samples to disk.",
    )
    video_group.set_defaults(save_videos=True)
    parser.add_argument(
        "--video_output_dir",
        type=str,
        default="./videos",
    ...
    parser.add_argument(
        ...
        type=str,
        default="wan_2_1",
        help="Prefix for generated video filenames.",
    )
    parser.add_argument(
        "--video_fps",
        type=int,
    ...
    parser.add_argument(
        ...
        default="rgb",
        choices=["rgb", "bgr"],
        help="Color format produced by the VAE output; frames are converted to RGB before saving.",
    )
