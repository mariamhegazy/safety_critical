...
def _compute_wan_latent_spec(args, transformer, vae):
    ...
    vae_scale_factor = getattr(vae, "vae_scale_factor_spatial", None)
...
def _prepare_image_conditioning_condition(
    ...
    vae_encode_scale = getattr(getattr(vae, "config", None), "scale_factor_spatial", None)
...
def _get_patch_multiple(transformer):
    cfg = getattr(_unwrap_module(transformer), "config", None)
    patch = getattr(cfg, "patch_size", 1)
    if isinstance(patch, int):
        return patch
    if isinstance(patch, (tuple, list)) and len(patch) >= 3:
        return patch[1]
    if isinstance(patch, (tuple, list)) and len(patch) == 2:
        return patch[0]
    return 1
...
def run_sample_step(
    ...
    for i in progress_bar:
        ...
        if torch.is_tensor(timestep_value):
            timestep_scalar = float(timestep_value.item())
            timestep_dtype = timestep_value.dtype
        else:
            timestep_scalar = float(timestep_value)
        ...
        if args.cfg_infer > 1:
            with torch.autocast("cuda", torch.bfloat16):
                latent_z = torch.cat([z, z], dim=0)  # z is fp32
                # IMPORTANT: uncond first, cond second
                cond_states = torch.cat(
                    [negative_prompt_embeds, encoder_hidden_states], dim=0
                )

                t = scheduler.timesteps[i]
                timesteps = t.expand(z.shape[0]).to(dtype=torch.long)
            ...
            pred = pred_uncond.to(torch.float32) + args.cfg_infer * (
                pred_text.to(torch.float32) - pred_uncond.to(torch.float32)
            )
        else:
            t = scheduler.timesteps[i]
            timesteps = t.expand(z.shape[0]).to(dtype=torch.long)
        ...
        # t = timestep_schedule[i]  # scalar tensor (long)
        t = scheduler.timesteps[i]
        step_out = scheduler.step(pred, t, z, return_dict=True)  # z fp32, pred fp32
        z_next = step_out.prev_sample
        z = _apply_inpaint_mask(z_next, condition)

        # stochasticity for GRPO (defines the transition density)
        sigma = sigma_schedule[i]
        sigma_next = sigma_schedule[i + 1]
        std = args.eta * torch.sqrt(torch.clamp(sigma - sigma_next, min=0.0))
        ...
        # log N(z_next ; mean, std^2 I)
        log_prob = (
            -0.5 * ((z_next - mean) / std) ** 2
            - torch.log(std)
            - 0.5 * math.log(2 * math.pi)
        )
        log_prob = log_prob.mean(dim=tuple(range(1, log_prob.ndim)))  # (B,)

        # z = z_next.to(torch.bfloat16)

        # # keep your mask logic the same
        # z = _apply_inpaint_mask(z, condition)
...
def sample_reference_model(
    ...
    if latent_spec is None:
        latent_spec = _compute_wan_latent_spec(args, transformer, vae)
    ...
    for index, batch_idx in enumerate(batch_indices):
        sched = UniPCMultistepScheduler.from_config(base_scheduler.config)
        sched.set_timesteps(args.sampling_steps, device=device)

        sigma_schedule = sched.sigmas.to(device=device, dtype=torch.float32)
        if sigma_schedule.shape[0] == args.sampling_steps:
            sigma_schedule = torch.cat(
                [sigma_schedule, sigma_schedule.new_zeros(1)], dim=0
            )

        timestep_schedule = sched.timesteps.to(device=device)
...
def main(args):
    ...
    vae.eval()
    # vae.enable_tiling()
    patch_multiple = _get_patch_multiple(transformer)
    vae_scale = getattr(vae, "vae_scale_factor_spatial", WAN5B_SPATIAL_DOWNSAMPLE)
    mod_value = int(vae_scale) * int(patch_multiple)

    args.h = max(mod_value, _round_down(args.h, mod_value))
    args.w = max(mod_value, _round_down(args.w, mod_value))
