...
try:  # Wan2.2 checkpoints ship a new WanModel class in newer diffusers releases
    from diffusers import WanModel
...
def load_transformer(
    ...
    elif model_type == "wan2.2_ti2v":
        if WanTransformer3DModel is None and WanModel is None:
            raise ImportError("diffusers build missing WAN support.")
        model_path = dit_model_name_or_path or pretrained_model_name_or_path
        transformer_dir = os.path.join(model_path, "transformer")
        if dit_model_name_or_path is None and os.path.isdir(transformer_dir):
            ...
            transformer_cls = WanModel or WanTransformer3DModel
...
def load_vae(model_type, pretrained_model_name_or_path):
    ...
    if model_type == "mochi":
        ...
        if AutoencoderKLWan is None:
            raise ImportError("diffusers build missing AutoencoderKLWan for WAN checkpoints.")
        vae_path = pretrained_model_name_or_path
        candidate_file = None
        if os.path.isfile(vae_path):
            candidate_file = vae_path
        else:
            maybe_file = os.path.join(vae_path, "Wan2.2_VAE.pth")
            if os.path.isfile(maybe_file):
                candidate_file = maybe_file
        if candidate_file is not None:
            load_fn = getattr(AutoencoderKLWan, "from_single_file", None)
            if load_fn is None:
                raise RuntimeError(
                    "AutoencoderKLWan.from_single_file not available; please upgrade diffusers."
                )
            vae = load_fn(candidate_file, torch_dtype=weight_dtype).to("cuda")
        else:
            vae = AutoencoderKLWan.from_pretrained(
                vae_path,
                subfolder="vae",
...
def get_no_split_modules(transformer):
    ...
    elif isinstance(transformer, WanTransformer3DModel) or (
        WanModel is not None and isinstance(transformer, WanModel)
    ):
        if WanTransformerBlock is None:
            raise ValueError("WanTransformerBlock not available in current diffusers build.")
