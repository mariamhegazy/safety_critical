...
# from fastvideo.models.hunyuan.vae import vae
...
def _compute_wan_latent_spec(args, transformer, vae):
    ...
    temporal_downsample = getattr(
        transformer_config, "temporal_compression_ratio", WAN5B_TEMPORAL_DOWNSAMPLE
    )
    ...
    patch_size = getattr(
        getattr(transformer_config, "config", transformer_config), "patch_size", None
    )
    if patch_size is None:
        patch_size = getattr(transformer_config, "patch_size", None)
    if isinstance(patch_size, int):
        ...
        if len(patch_size) == 3:
            ...
            patch = (1, patch_size[0], patch_size[1])
        else:
            patch = (1, 1, 1)
    else:
        patch = (1, 2, 2)
    latent_t = ((args.t - 1) // temporal_downsample) + 1
    latent_h = args.h // vae_scale_factor
    latent_w = args.w // vae_scale_factor

    expand_timesteps = getattr(transformer_config, "expand_timesteps", None)
    if expand_timesteps is None:
        model_index_path = os.path.join(
            args.pretrained_model_name_or_path, "model_index.json"
        )
...
def _get_vae_scaling_factor(vae):
    ...
    config_factor = getattr(getattr(vae, "config", None), "scaling_factor", None)
...
def _retrieve_latents(encoder_output):
    ...
    raise AttributeError("Encoder output does not contain latents.")


def _prepare_image_conditioning_condition(
    image_meta,
    args,
    latent_spec,
    vae,
    ...
    if not image_meta or all(meta is None for meta in image_meta):
        return None
    ...
    vae_scale_factor = latent_spec["vae_scale_factor"]
    latent_t = latent_spec["latent_t"]
    target_h = latent_spec["latent_h"]
    target_w = latent_spec["latent_w"]

    latent_h, latent_w = latent_spec["latent_h"], latent_spec["latent_w"]
    patch = latent_spec["patch_size"]
    ph, pw = patch[1], patch[2]
    # print("vae_scale_factor:", latent_spec["vae_scale_factor"])
    # print("latent_hw:", (latent_h, latent_w))
    # print("patch_hw:", (ph, pw))
    # print("tokens/frame:", (math.ceil(latent_h / ph), math.ceil(latent_w / pw)))
    ...
    vae_encode_scale = getattr(
        getattr(vae, "config", None), "scale_factor_spatial", None
    )
    if vae_encode_scale is None:
        vae_encode_scale = getattr(vae, "vae_scale_factor_spatial", vae_scale_factor)
    vae_encode_scale = int(vae_encode_scale)
    encode_height = int(target_h) * vae_encode_scale
    encode_width = int(target_w) * vae_encode_scale

    video_processor = VideoProcessor(vae_scale_factor=vae_encode_scale)
    image_tensor = video_processor.preprocess(
        images,
        height=encode_height,
        width=encode_width,
    ).to(device_obj, dtype=torch.float32)

    first_frame = image_tensor.to(device=device_obj, dtype=vae.dtype).unsqueeze(2)

    with torch.inference_mode():
        with torch.autocast(device_obj.type, dtype=vae.dtype):
            latents_first = _retrieve_latents(vae.encode(first_frame))

    latents_first = latents_first.to(device_obj, dtype=torch.float32)
    latents_mean = torch.as_tensor(
        vae.config.latents_mean, device=device_obj, dtype=torch.float32
    ).view(1, vae.config.z_dim, 1, 1, 1)
    latents_std = 1.0 / torch.as_tensor(
        vae.config.latents_std, device=device_obj, dtype=torch.float32
    ).view(1, vae.config.z_dim, 1, 1, 1)
    latents_first = (latents_first - latents_mean) * latents_std

    B, C, T, H, W = latents_first.shape

    if H != target_h or W != target_w:
        latents_first = F.interpolate(
            latents_first,
    ...
    # print("shape of latents_first: ", latents_first.shape)
    # print("shape of first_image: ", first_frame.shape)

    known_latents = latents_first.new_zeros((B, C, latent_t, target_h, target_w))
    known_latents[:, :, 0:1] = latents_first
    mask = torch.ones(
        (B, 1, latent_t, target_h, target_w),
        device=device_obj,
        dtype=torch.float32,
    )
    mask[:, :, 0] = 0.0

    if getattr(args, "save_conditioning_preview", False) and not getattr(
        args, "_conditioning_preview_saved", False
    ):
        with torch.no_grad():
            decode_latents = latents_first / latents_std + latents_mean
            decode_latents = decode_latents.to(device_obj, dtype=vae.dtype)
            recon = vae.decode(decode_latents, return_dict=False)[0]
        recon_frames = video_processor.postprocess_video(recon)
        preview_dir = Path(
            getattr(args, "conditioning_preview_dir", "./conditioning_previews")
        )
        preview_dir.mkdir(parents=True, exist_ok=True)
        frame = recon_frames[0]
        ...
        preview_path = preview_dir / f"conditioning_preview_rank{rank:02d}.png"
        Image.fromarray(frame_uint8).save(preview_path)
        setattr(args, "_conditioning_preview_saved", True)

    return {
        "mask": mask.to(torch.bfloat16),
        "known_latents": known_latents.to(torch.bfloat16),
    }


def _condition_select(condition, indices):
...
def _condition_repeat(condition, repeats):
    if condition is None:
        return None
    repeated = {}
    for key, value in condition.items():
        if isinstance(value, torch.Tensor):
            repeated[key] = torch.repeat_interleave(value, repeats, dim=0)
        else:
            repeated[key] = value
    return repeated


def _condition_duplicate(condition, times=2):
    if condition is None:
        return None
    duplicated = {}
    for key, value in condition.items():
        if isinstance(value, torch.Tensor):
            duplicated[key] = torch.cat([value] * times, dim=0)
        else:
...
# def _prepare_conditioned_latents(latents, timesteps, scheduler, condition):
#     latents_fp32 = latents.to(torch.float32)
#     latents_fp32 = _apply_inpaint_mask(latents_fp32, condition)

#     latents_fp32 = scheduler.scale_model_input(latents_fp32, timesteps)
#     return latents_fp32.to(torch.bfloat16)


def _prepare_conditioned_latents(latents, timesteps, scheduler, condition):
    x = latents.to(torch.float32)

    # inject condition in the SAME space as x
    if condition is not None:
        mask = condition["mask"].to(x.device, dtype=x.dtype)
        known = condition["known_latents"].to(x.device, dtype=x.dtype)
        x = (1.0 - mask) * known + mask * x

    # now apply the scheduler scaling (uniformly)
    x = scheduler.scale_model_input(x, timesteps)
    return x.to(torch.bfloat16)


# def _prepare_transformer_timesteps(timesteps, latents, condition, latent_spec):
#     if not torch.is_tensor(timesteps):
#         timesteps = torch.as_tensor(
#             timesteps, device=latents.device, dtype=torch.float32
#         )
#     else:
#         timesteps = timesteps.to(latents.device)
#     if condition is None:
#         return timesteps
#     mask = condition["mask"][:, 0].to(latents.device, dtype=torch.float32)
#     patch = latent_spec.get("patch_size", (1, 2, 2))
#     patch_h = patch[1] if len(patch) > 1 else 1
#     patch_w = patch[2] if len(patch) > 2 else patch_h
#     mask_tokens = mask[:, :, ::patch_h, ::patch_w].reshape(latents.shape[0], -1)
#     tokens_per_frame = (
#         math.ceil(latent_spec["latent_h"] / patch_h)
#         * math.ceil(latent_spec["latent_w"] / patch_w)
#     )
#     seq_len = latent_spec["latent_t"] * tokens_per_frame
#     if mask_tokens.shape[1] < seq_len:
#         pad = mask_tokens.new_ones(mask_tokens.shape[0], seq_len - mask_tokens.shape[1])
#         mask_tokens = torch.cat([mask_tokens, pad], dim=1)
#     elif mask_tokens.shape[1] > seq_len:
#         mask_tokens = mask_tokens[:, :seq_len]
#     timestep_tokens = mask_tokens * timesteps.to(mask_tokens.dtype).view(-1, 1)
#     return timestep_tokens.to(torch.float32)


def _prepare_transformer_timesteps(timesteps, latents, condition, latent_spec):
    timesteps = timesteps.to(latents.device)
    if condition is None or not latent_spec.get("expand_timesteps", False):
        return timesteps  # (B,)
    ...
    # follow diffusers: use mask from batch 0, flatten in (T, H', W') order
    mask0 = condition["mask"][0, 0].to(latents.device, dtype=torch.float32)  # (T,H,W)
    t0 = timesteps[0].to(torch.float32)

    ts_tokens = (mask0[:, ::patch_h, ::patch_w] * t0).flatten()  # (seq_len,)
    return ts_tokens.unsqueeze(0).expand(latents.shape[0], -1)  # (B, seq_len)


def _apply_inpaint_mask(latents, condition):
    if condition is None:
        return latents
    mask = condition["mask"].to(latents.device, dtype=latents.dtype)
    known_latents = condition["known_latents"].to(latents.device, dtype=latents.dtype)
    return (1.0 - mask) * known_latents + mask * latents
...
def _reshape_vae_latent_stats(vae, latents):
    ...
    view = (1, vae.config.z_dim) + (1,) * (latents.ndim - 2)
    return latents_mean.view(view), latents_std.view(view)
...
def flux_step(
    ...
    is_last = index == sigmas.shape[0] - 2
...
def _wan_boundary_step(num_inference_steps, boundary_ratio):
    if num_inference_steps is None:
        return None
    if torch.is_tensor(num_inference_steps):
    ...
    boundary = int(boundary_ratio * num_inference_steps)
    return max(0, min(num_inference_steps, boundary))


def _select_wan_transformer(args, step_index, transformer, transformer_2, num_inference_steps):
    if transformer_2 is None:
        return transformer
    boundary_ratio = getattr(args, "wan_boundary_ratio", None)
    if boundary_ratio is None:
        boundary_ratio = 0.9
    boundary = _wan_boundary_step(num_inference_steps, boundary_ratio)
    if boundary is None:
        return transformer_2
    if torch.is_tensor(step_index):
        step_index = int(step_index.item())
    else:
        step_index = int(step_index)
    # Mirror common Wan2.2 usage: transformer_2 is the "high-noise" expert used
    # in early steps; transformer is used after the boundary.
    return transformer_2 if step_index < boundary else transformer
...
def run_sample_step(
    ...
    z = _apply_inpaint_mask(z, condition)
    all_latents = [z]
    all_log_probs = []
    ...
    for i in progress_bar:
        ...
        if not torch.is_tensor(timestep_value):
            timestep_value = torch.tensor(timestep_value, device=z.device)

        timestep_value = timestep_value.to(
            device=z.device, dtype=timestep_schedule.dtype
        )
        timesteps = timestep_value.expand(B)
        active_transformer = _select_wan_transformer(
            args, i, transformer, transformer_2, args.sampling_steps
        )
        if args.cfg_infer > 1:
            with torch.autocast("cuda", torch.float32):
                ...
                duplicated_condition = _condition_duplicate(condition)

                scaled_inputs = _prepare_conditioned_latents(
                    latent_z, timestep_inputs, scheduler, duplicated_condition
                )
                transformer_timesteps = _prepare_transformer_timesteps(
                    timestep_inputs, latent_z, duplicated_condition, latent_spec
                )
                pred = active_transformer(
                    hidden_states=scaled_inputs,
                    timestep=transformer_timesteps,
            ...
            with torch.autocast("cuda", torch.bfloat16):
                model_inputs = _prepare_conditioned_latents(
                    z, timesteps, scheduler, condition
                )
                transformer_timesteps = _prepare_transformer_timesteps(
                    timesteps, z, condition, latent_spec
                )
                pred = active_transformer(
        ...
        assert latent_spec["channels"] == z.shape[1]
        ...
        if compare_scheduler and i < compare_limit:
            cmp_prev = cmp_scheduler.step(
                ...
                torch.tensor(
                    timestep_scalar,
                    device=z.device,
                    dtype=cmp_scheduler.timesteps.dtype,
                ),
                cmp_latents,
                return_dict=False,
            )[0]
    ...
    latents = _apply_inpaint_mask(z, condition)
    latents = pred_original
    latents = _apply_inpaint_mask(pred_original, condition)
...
def grpo_one_step(
    ...
    if args.cfg_infer > 1:
        ...
        with torch.autocast("cuda", torch.bfloat16):
            model_inputs = _prepare_conditioned_latents(
                latents, timesteps, scheduler, condition
            )
            transformer_timesteps = _prepare_transformer_timesteps(
                timesteps, latents, condition, latent_spec
            )
            pred = active_transformer(
                hidden_states=model_inputs,
                timestep=transformer_timesteps,
...
def sample_reference_model(
    ...
    if latent_spec is None:
        latent_spec = _compute_wan_latent_spec(
            args, transformer_2 if transformer_2 is not None else transformer, vae
        )
    ...
    latent_channels = latent_spec["channels"]
    latent_t = latent_spec["latent_t"]
    latent_h = latent_spec["latent_h"]
    latent_w = latent_spec["latent_w"]

    # vae.enable_tiling()

    video_processor = VideoProcessor(vae_scale_factor=latent_spec["vae_scale_factor"])
    ...
    video_filename_prefix = getattr(args, "video_filename_prefix", "wan14b")
    ...
    shared_latents = None
    if args.init_same_noise and image_latents is None:
        shared_latents = _sample_latents(
    ...
    for index, batch_idx in enumerate(batch_indices):
        ...
        batch_conditioning = (
            _condition_select(image_latents, batch_idx)
            if image_latents is not None
            else None
        )

        if not args.init_same_noise or shared_latents is None:
            current_latents = _sample_latents(
                (1, latent_channels, latent_t, latent_h, latent_w)
            )
        else:
            current_latents = shared_latents.clone()
        ...
        with torch.inference_mode():
            with torch.autocast("cuda", dtype=torch.bfloat16):
                ...
                decode_latents = _apply_inpaint_mask(latents, batch_conditioning)
...
def train_one_step(
    ...
    latent_spec = getattr(args, "_wan_latent_spec", None)
    if latent_spec is None:
        latent_spec = _compute_wan_latent_spec(
            args, transformer_2 if transformer_2 is not None else transformer, vae
        )
        setattr(args, "_wan_latent_spec", latent_spec)
    image_conditioning = None
    if image_meta and any(meta is not None for meta in image_meta):
        image_conditioning = _prepare_image_conditioning_condition(
            image_meta, args, latent_spec, vae, device
        )
        image_conditioning = {
            key: value.detach() if isinstance(value, torch.Tensor) else value
            for key, value in image_conditioning.items()
        }
    # image_conditioning = None
    ...
    if args.use_group:
        ...
        image_conditioning = _condition_repeat(image_conditioning, args.num_generations)
    ...
    (
        ...
        caption,
        inferencer,
        image_latents=image_conditioning,
    ...
    if dist.get_rank() == 0:
        ...
        with open("./reward_wan14b.txt", "a") as f:
    ...
    condition_slices = _split_condition(image_conditioning)
    if condition_slices is not None:
        for sample_dict, cond in zip(samples_batched_list, condition_slices):
            sample_dict["image_condition"] = cond
    else:
        for sample_dict in samples_batched_list:
            sample_dict["image_condition"] = None
    ...
    for i, sample in list(enumerate(samples_batched_list)):
        for _ in range(train_timesteps):
            clip_range = 1e-4
            adv_clip_max = 5.0
            condition = sample.get("image_condition")
        ...
        if (i + 1) % args.gradient_accumulation_steps == 0:
            trainable_transformer = transformer_2 if transformer_2 is not None else transformer
            grad_norm = trainable_transformer.clip_grad_norm_(max_grad_norm)
            optimizer.step()
            lr_scheduler.step()
            optimizer.zero_grad()
...
def main(args):
    ...
    if model_type == "wan2.2_14b":
        model_type = "wan2.2_ti2v"
    main_print(f"--> loading model type {args.model_type} (canonical: {model_type})")
    master_dtype = torch.float32 if args.master_weight_type == "fp32" else torch.bfloat16

    transformer_2_dir = os.path.join(args.pretrained_model_name_or_path, "transformer_2")
    has_transformer_2 = os.path.isdir(transformer_2_dir)
    ...
    if boundary_ratio is None:
        ...
        if os.path.exists(model_index_path):
            try:
                with open(model_index_path, "r", encoding="utf-8") as f:
                    model_index = json.load(f)
                boundary_ratio = float(model_index.get("boundary_ratio", boundary_ratio))
            except (OSError, json.JSONDecodeError, TypeError, ValueError):
                boundary_ratio = 0.9
    args.wan_boundary_ratio = boundary_ratio
    ...
    if lora_target_modules is None:
        ...
        lora_target_modules = [
            item.strip() for item in lora_target_modules.split(",") if item.strip()
        ]

    transformer_2 = None
    if has_transformer_2 and model_type == "wan2.2_ti2v":
        ...
        if args.use_lora:
            ...
            transformer_2.add_adapter(transformer_lora_config)
            transformer_2.config.lora_rank = args.lora_rank
            transformer_2.config.lora_alpha = args.lora_alpha
            transformer_2.config.lora_target_modules = lora_target_modules
    else:
    ...
    trainable_transformer = transformer_2 if transformer_2 is not None else transformer
    main_print(
        f"  Total training parameters = {sum(p.numel() for p in trainable_transformer.parameters() if p.requires_grad) / 1e6} M"
    )
    main_print(
    ...
    train_no_split_modules = no_split_modules
    if transformer_2 is not None:
        fsdp_kwargs_2, no_split_modules_2 = get_dit_fsdp_kwargs(
            transformer_2,
            args.fsdp_sharding_startegy,
            args.use_lora,
            args.use_cpu_offload,
        ...
        if args.use_lora and fsdp_kwargs_2.get("auto_wrap_policy") is not None:
            transformer_2._no_split_modules = [
                no_split_module.__name__ for no_split_module in no_split_modules_2
            ]
            fsdp_kwargs_2["auto_wrap_policy"] = fsdp_kwargs_2["auto_wrap_policy"](transformer_2)
        transformer_2 = FSDP(transformer_2, **fsdp_kwargs_2)
        train_no_split_modules = no_split_modules_2
    ...
    if args.gradient_checkpointing:
        apply_fsdp_checkpointing(
            transformer_2 if transformer_2 is not None else transformer,
            train_no_split_modules,
            args.selective_checkpointing,
        )

    # Set models as train/eval.
    if transformer_2 is not None:
        transformer.eval()
        transformer_2.train()
    else:
    ...
    params_to_optimize = list(
        filter(lambda p: p.requires_grad, trainable_transformer.parameters())
    )
    ...
    vae, autocast_type, fps = load_vae(model_type, args.vae_model_path)
    ...
    trainable_transformer = transformer_2 if transformer_2 is not None else transformer
    main_print(
        f"  Total training parameters per FSDP shard = {sum(p.numel() for p in trainable_transformer.parameters() if p.requires_grad) / 1e9} B"
    )
    # print dtype
    main_print(f"  Master weight dtype: {next(trainable_transformer.parameters()).dtype}")
    ...
    for epoch in range(1):
        ...
        for step in range(init_steps + 1, args.max_train_steps + 1):
            ...
            if step % args.checkpointing_steps == 0:
                checkpoint_transformer = transformer_2 if transformer_2 is not None else transformer
                save_checkpoint(checkpoint_transformer, rank, args.output_dir, step, epoch)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="data/Wan2.2-I2V-A14B-Diffusers",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Boundary ratio for routing between Wan2.2 transformer experts; defaults to model_index.json.",
    )
    ...
    lora_group.add_argument(
        ...
        dest="use_lora",
        action="store_true",
        help="Enable LoRA finetuning (default for Wan2.2 A14B: applies to transformer_2 when present).",
    )
    lora_group.add_argument(
        "--no_lora",
        dest="use_lora",
        action="store_false",
        help="Disable LoRA and finetune transformer_2 weights directly (transformer_1 remains frozen).",
    )
    lora_group.set_defaults(use_lora=True)
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Comma-separated LoRA target module names (applies to transformer_2 when present).",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Decode and save the conditioned first frame for inspection.",
    )
    parser.add_argument(
        "--conditioning_preview_dir",
        type=str,
        default="./conditioning_previews",
        help="Directory for saved conditioning preview images.",
    )
    parser.add_argument(
        "--sampler_seed",
        type=int,
